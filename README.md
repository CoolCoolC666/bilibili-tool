...
我在 v2 跑过 `杀哥事件视频数据-完整版.xlsx`（89 条 / 86 个独立 BV），结果：
...
符合预期——杀哥事件 4K 修复版 80%+ 已下架，B 站返回 62002 稿件不可见 / -404 视频不存在，新版正确标记且不中断。
...
- 杀哥事件 4K 修复版和春山响相关视频的创作者——这两份测试数据帮我验证了 80%+ 失效场景下脚本不会崩
